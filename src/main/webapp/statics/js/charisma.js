$(document).ready(function(){
	...
	$('#modifySavePassword').click(function () {
		...
		if(oldPwd==""){
            ...
            tip.css("color","red");
            tip.html("对不起，新密码的长度不低于6位,谢谢！");
            $("#newpassword").focus;
		}else if(aginPwd==""){
            tip.css("color","red");
            tip.html("对不起，请输入二次密码,谢谢！");
            $("#aginPwd").focus;
		}else if(newPwd!=aginPwd){
            tip.css("color","red");
            tip.html("对不起，两次密码不一致,请重新输入，谢谢！");
            $("#aginPwd").focus;
		}else{
			var user=new Object();
			user.password=oldPwd;
			user.password2=aginPwd;
			$.ajax({
				type:"POST",
				url:"/backend/passwordModify.html",
	...
	/*主菜单列表开始*/
	var result='';
	var jsonList=eval('('+menuList+')');
	//alert(jsonList);
	for (var i=0;i<jsonList.length;i++){
        //配置一级菜单
        result = result + '<li class="nav-header hidden-tablet" onclick="$(\'#test'+i+'\').toggle(500);" style="cursor:pointer">'+jsonList[i].mainMenu.functionName+'</li>';
        //配置二级菜单
        ...
        for(var j = 0;j < jsonList[i].subMenus.length;j++){
            ...
            result = result + "<li><a class=\"ajax-link\" href=\""+jsonList[i].subMenus[j].funcUrl+"\"><i class="+pic+"></i> <span class=\"hidden-tablet\">"+jsonList[i].subMenus[j].functionName+"</span></a></li>";
        }
        result = result + "</ul></li>";
    }
    $("#menus").append(result);
